...
def fan_to_cone_beam_geometry(fb_geometry, det_z_span=1.):
    ...
    det_min_pt = [-det_z_span/2., fb_geometry.det_partition.min_pt[0]]
    det_max_pt = [+det_z_span/2., fb_geometry.det_partition.max_pt[0]]
    dpart = odl.discr.RectPartition(
        odl.set.IntervalProd(det_min_pt, det_max_pt),
        odl.discr.RectGrid([0.], fb_geometry.det_partition.coord_vectors[0]))
    ...
    det_axes_init = ([1., 0., 0.],
                     [0.,
                      fb_geometry.det_axis_init[0],
                      fb_geometry.det_axis_init[1]],
                     )
    translation = [0.,
                   fb_geometry.translation[0],
...
def parallel_2d_to_3d_geometry(pb2d_geometry, det_z_shape=1, det_z_span=None):
    ...
    det_min_pt = [det_z_partition.min_pt[0],
                  pb2d_geometry.det_partition.min_pt[0]]
    det_max_pt = [det_z_partition.max_pt[0],
                  pb2d_geometry.det_partition.max_pt[0]]
    dpart = odl.discr.RectPartition(
        odl.set.IntervalProd(det_min_pt, det_max_pt),
        odl.discr.RectGrid(det_z_partition.coord_vectors[0],
    ...
    det_pos_init = [0.,
                    pb2d_geometry.det_pos_init[0],
                    pb2d_geometry.det_pos_init[1]]
    det_axes_init = ([1., 0., 0.],
                     [0.,
                      pb2d_geometry.det_axis_init[0],
                      pb2d_geometry.det_axis_init[1]])
    translation = [0.,
                   pb2d_geometry.translation[0],
...
def discretized_space_2d_to_3d(space, z_shape=1, z_span=None):
    ...
    rect_partition = odl.discr.RectPartition(
            odl.set.IntervalProd(
                ...
                [z_partition.max_pt[0],
                 space.max_pt[0],
                 space.max_pt[1]]),
            odl.discr.RectGrid(z_partition.coord_vectors[0],
    ...
    if version.parse(odl.__version__) <= version.parse('0.7.0'):
        ...
        function_space = odl.FunctionSpace(
            odl.IntervalProd([z_partition.min_pt[0],
